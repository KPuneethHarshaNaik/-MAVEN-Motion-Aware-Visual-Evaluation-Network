...
Two data sources are combined:
  1. MMASDBinaryDataset  — MMASD ROMP-2D pre-computed skeleton sequences
                           (folder name: ..._y = ASD, ..._n = TD, ..._i = skip)
  2. PreExtractedDataset — MediaPipe-extracted skeleton NPZ files from
                           autism_data_anonymized (cached by extract_poses.py)

Both return tensors of shape (2, MAX_FRAMES, 24)  — (C, T, V) — so they can
be combined in a single DataLoader via ConcatDataset.

Data Augmentation (applied only during training):
  • Horizontal flip (mirror left/right joints)
  • Gaussian jitter on joint positions
  • Random temporal crop + re-sampling
  • Random temporal speed perturbation
  • Random joint dropout (zero out 1-2 joints)
"""
...
# ─── Horizontal mirror mapping for SMPL-24 ────────────────────────────────────
# When we flip x-axis we swap left↔right joints
_SMPL24_MIRROR = {
    1: 2,  2: 1,    # L_Hip ↔ R_Hip
...
# ─── Dataset 1: MMASD ROMP-2D skeleton data ───────────────────────────────────
...
class MMASDBinaryDataset(Dataset):
    ...
    where label ∈ {'y'=ASD, 'n'=TD, 'i'=skip-indeterminate}.
...
# ─── Dataset 2: Pre-extracted MediaPipe skeletons ─────────────────────────────
...
# ─── Dataset 3:  Raw video dataset (on-the-fly extraction) ────────────────────
...
# ─── Helper: build train / val / test splits ──────────────────────────────────
...
def build_splits(
    ...
    Strategy:
      • MMASD sequences are split 75 / 15 / 10 by sequence (subject-agnostic).
      • autism_data pre-extracted NPZ:
            train  → training_set/ASD + training_set/TD (split internally)
            test   → testing_set/ASD + testing_set/TD
...
# ─── Internal: index-based subset wrapper ─────────────────────────────────────
